    ...
    RESET: OutputPin,
{
    ...
    pub fn get_ready_packet_size(&mut self) -> Result<u8, Error<E, CS::Error, RESET::Error>> {
        self.read_register(Register::RegRxNbBytes)
    }
    ...
    pub fn transmitting(&mut self) -> Result<bool, Error<E, CS::Error, RESET::Error>> {
        let op_mode = self.read_register(Register::RegOpMode)?;
        if (op_mode & RadioMode::Tx.addr()) == RadioMode::Tx.addr()
            || (op_mode & RadioMode::FsTx.addr()) == RadioMode::FsTx.addr()
        {
            Ok(true)
        } else {
            if (self.read_register(Register::RegIrqFlags)? & IRQ::IrqTxDoneMask.addr()) == 1
            {
                self.write_register(Register::RegIrqFlags, IRQ::IrqTxDoneMask.addr())?;
            }
            Ok(false)
        }
    }
    ...
    pub fn set_tx_power(
        ...
        if PaConfig::PaOutputRfoPin.addr() == output_pin {
            ...
            self.write_register(Register::RegPaConfig, (0x70 | level) as u8)
        } else {
            // PA BOOST
            if level > 17 {
                if level > 20 {
    ...
    pub fn set_signal_bandwidth(
        ...
        if bw == 9 {
            if self.frequency < 525 {
                self.write_register(Register::RegHighBWOptimize1, 0x02)?;
                self.write_register(Register::RegHighBWOptimize2, 0x7f)?;
            } else {
                self.write_register(Register::RegHighBWOptimize1, 0x02)?;
                self.write_register(Register::RegHighBWOptimize2, 0x64)?;
            }
        } else {
            self.write_register(Register::RegHighBWOptimize1, 0x03)?;
            self.write_register(Register::RegHighBWOptimize2, 0x65)?;
        }
